// SubsidySeva - Subsidies Data
// Contains authentic government subsidy data for static website
...
function getAllCategories() {
    ...
    SUBSIDIES_DATA.forEach(subsidy => {
        subsidy.categories.forEach(category => {
...
// Filter subsidies by category
function filterByCategory(subsidies, category) {
    if (!category) return subsidies;
    return subsidies.filter(subsidy => 
        subsidy.categories.includes(category)
    );
}

// Search subsidies by text
function searchSubsidies(subsidies, query) {
    if (!query) return subsidies;
    const searchTerm = query.toLowerCase();
    return subsidies.filter(subsidy => 
        subsidy.title.toLowerCase().includes(searchTerm) ||
        subsidy.description.toLowerCase().includes(searchTerm) ||
        subsidy.eligibility.toLowerCase().includes(searchTerm)
    );
}

// Get active subsidies
function getActiveSubsidies() {
    return SUBSIDIES_DATA.filter(subsidy => subsidy.status === 'Active');
}